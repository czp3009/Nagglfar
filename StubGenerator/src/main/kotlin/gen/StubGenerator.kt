...
fun kotlinHeader(packageName: String) = """
    package $packageName
...
fun wasmFunctionName(functionName: String, interfaceName: String) = "knjs__${interfaceName}_$functionName"

fun wasmSetterName(propertyName: String, interfaceName: String) = "knjs_set__${interfaceName}_$propertyName"

fun wasmGetterName(propertyName: String, interfaceName: String) = "knjs_get__${interfaceName}_$propertyName"

val Operation.kotlinTypeParameters: String
    get() {
...
val Interface.wasmReceiverArgs
    get() =
...
val Interface.wasmTypedReceiverArgs
    get() =
        if (isGlobal) emptyList()
...
fun Arg.wasmTypedMapping() = this.wasmArgNames().map { "$it: Int" }.joinToString(", ")
...
fun List<Arg>.wasmTypedMapping() = this.map(Arg::wasmTypedMapping)

// TODO: more complex return types, such as returning a pair of Ints
// will require a more complex approach.
fun Type.wasmReturnTypeMapping() = if (this == idlVoid) "Unit" else "Int"
...
val Interface.wasmReceiverArgName
    get() =
...
val Operation.wasmReturnArgName
    get() =
        returnType.wasmReturnArgName

val Attribute.wasmReturnArgName
    get() =
        type.wasmReturnArgName

val Type.wasmReturnArgName
    get() =
        when (this) {
...
fun main() {
    File("StubGenerator/dist").run {
        ...
        resolve("kotlin_stubs.kt").run {
            createNewFile()
            writeText(generateKotlin("screeps", idlScreeps))
        }

        resolve("js_stubs.js").run {
            createNewFile()
            writeText(generateJs(idlScreeps))
        }
    }
}
